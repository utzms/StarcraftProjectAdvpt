...
template <class Race> template <class EntityType> std::shared_ptr< std::vector<std::pair<bool, std::shared_ptr<Requirements>>>> TechnologyManager<Race>::requestRequirements(std::shared_ptr<EntityType> entity) 
{
    ...
    auto res = std::shared_ptr<std::vector<std::pair<bool, std::shared_ptr<Requirements>>>>(new std::vector<std::pair<bool, std::shared_ptr<Requirements>>>);
    for(auto tech : techVec) 
    {
        if(TechnologyManager<Race>::getNeededRequirements(tech, res))
        {
            return true;
        }
    }
